...
function Shop() {
  ...
  // --- NÂNG CẤP QUAN TRỌNG: KHỞI TẠO STATE TỪ URL ---
  // Thay vì để mặc định, ta đọc URL ngay lập tức
  const [filters, setFilters] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    return {
      search: params.get("search") || "",
      category: params.get("category") || "all", // Lấy category từ URL
  ...
  // Khi URL thay đổi (ví dụ user bấm back/forward), cập nhật lại state
  useEffect(() => {
    ...
    setFilters(prev => ({
      ...
      search: params.get("search") || "",
      skip: 0 // Reset skip về 0 khi đổi trang
    }));
    // Lưu ý: Việc setFilters sẽ kích hoạt useEffect "gọi API" bên dưới
  }, [location.search]);

  // Lấy danh sách Categories (Sidebar)
  useEffect(() => {
  ...
  // Gọi API khi bộ lọc thay đổi (Debounce)
  useEffect(() => {
    const timeout = setTimeout(() => {
      setLoading(true);
      // Nếu không phải đang load more thì reset skip về 0 logic ở đây
      if (!loadingMore) {
         // fetchProducts lo phần reset
      }
      fetchProducts(false);
    }, 500);
